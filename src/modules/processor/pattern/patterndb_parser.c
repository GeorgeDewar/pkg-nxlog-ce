...
#define XCC_VERSION_STRING  "xcc-0.5.1"
...
int xcc_run(FILE *fp, void **root, void *udata, XCCParserData *oldpdata,
              ...
              XCCExceptionHandler exception_handler)
{
    ...
    if (oldpdata == NULL) {
        ...
        pdata->cbufsize = 0;
        pdata->cbuflen  = 0;
        pdata->includes = xcc_stack_new((XCC_stack_data_free) free);
	...
	pdata->end_handler = end_element_handler;
	pdata->start_handler = start_element_handler;
    }
    else {
        pdata = oldpdata;
	XML_SetElementHandler(xp, pdata->start_handler, pdata->end_handler);
    }
    ...
    while (!pdata->error) {
        ...
        if (!XML_Parse(xp, Buff, len, done)) {
            void *file = NULL;
            xcc_stack_get_last(pdata->includes, (void **) &file);
            if (file == NULL) {
                ...
                xcc_error("Parse error in %s at line %d:\n\t%s",
                          (char *) file,
                          XML_GetCurrentLineNumber(xp),
                          XML_ErrorString(XML_GetErrorCode(xp)));
            }
...
static void xcc_start_handler(void *data, const char *el, const char **attr)
{
    XCCParserData *pdata = (XCCParserData *) data;
...
{
    if (xcc_run(fp, root, udata, NULL, xcc_start_handler, xcc_end_handler, exception_handler)
        != XCC_RETURN_SUCCESS) {
        return XCC_RETURN_FAILURE;
    }
    return XCC_RETURN_SUCCESS;
