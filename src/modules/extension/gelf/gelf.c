...
nx_string_t *nx_logdata_to_gelf(nx_gelf_ctx_t *ctx)
{
    ...
    ASSERT(yajl_gen_string(gen, (const unsigned char *) "1.0", 3) == yajl_gen_status_ok);
	...
	ASSERT(yajl_gen_string(gen, (const unsigned char *) message->string->buf,
			       message->string->len > 64 ? 64 : message->string->len) == yajl_gen_status_ok);
	...
	ASSERT(yajl_gen_string(gen, (const unsigned char *) message->string->buf,
			       message->string->len) == yajl_gen_status_ok);
    }
    else
    {
	ASSERT(ctx->logdata->raw_event != NULL);
	ASSERT(yajl_gen_string(gen, (const unsigned char *) "short_message", 13) == yajl_gen_status_ok);
	ASSERT(yajl_gen_string(gen, (const unsigned char *) ctx->logdata->raw_event->buf,
			       ctx->logdata->raw_event->len > 64 ? 64 : ctx->logdata->raw_event->len)
	       == yajl_gen_status_ok);

	ASSERT(yajl_gen_string(gen, (const unsigned char *) "full_message", 12) == yajl_gen_status_ok);
	ASSERT(yajl_gen_string(gen, (const unsigned char *) ctx->logdata->raw_event->buf,
			       ctx->logdata->raw_event->len) == yajl_gen_status_ok);
    }
